#!/usr/bin/env python3
"""
EdgeTyper — Aggregate (v4, robust)
 - Finds replica artifacts even when nested (e.g., runs/<id>/…).
 - If availability CSVs are missing/header-only, recomputes them from:
     (a) edges.parquet + pred_ours.csv, else
     (b) graph.json → rebuild edges/preds, then recompute.
 - Derives entrypoints.txt (from existing txt/csv or edges).
...
def _find_first(root: Path, name: str) -> Optional[Path]:
    """Find the first file named `name` within depth<=3 under root."""
...
def _guess_entrypoints_from_edges(edges_parquet: Path) -> list[str]:
    ...
    if not eps:
        ...
        eps = hl[:3] if hl else []
    if not eps: eps = src.value_counts().sort_values(ascending=False).index.tolist()[:3]
...
def ensure_entrypoints(replica_dir: Path) -> Path:
    ...
    if not txt.exists():
        nested_txt = _find_first(replica_dir, "entrypoints.txt")
        if nested_txt: shutil.copyfile(nested_txt, txt)
...
def ensure_availability(replica_dir: Path, pgrid: list[float]) -> Tuple[Optional[Path], Optional[Path]]:
    ...
    if (not edges.exists()) or (not pred.exists()):
        if not _rebuild_from_graph_json(replica_dir):
            print(f"::warning ::[{replica_dir.name}] missing edges/parquet or pred_ours; cannot recompute")
...
def build_aggregate(replicas: list[Path], site: Path) -> None:
    ...
    html = (
        ...
        "<h2>Downloads</h2><ul>"
        "<li><a href='data/aggregate_summary.json'>aggregate_summary.json</a></li>"
        "<li><a href='data/replicas_summary.csv'>replicas_summary.csv</a></li>"
        "<li><a href='data/availability_aggregate.json'>availability_aggregate.json</a></li>"
        "<li><a href='data/availability_aggregate.csv'>availability_aggregate.csv</a></li>"
        "<li><a href='data/availability_typed_all.csv'>availability_typed_all.csv</a></li>"
        "<li><a href='data/availability_block_all.csv'>availability_block_all.csv</a></li>"
        "<li><a href='data/availability_live_all.csv'>availability_live_all.csv</a></li>"
        "</ul>"
    ...
    print(f"[aggregate_site_v4] wrote {site/'index.html'} and {data/'availability_aggregate.csv'}")
